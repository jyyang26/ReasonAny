...
class CustomLogitsProcessor(LogitsProcessor):
    def __init__(self, token_to_suppress, strategy="zero"):
...
@torch.no_grad()
def generate_completions_logitsprocessor(model, tokenizer, prompts, batch_size=1, stop_id_sequences=None, add_special_tokens=True, disable_tqdm=False, **generation_kwargs):
    print('This is func [generate_completions_logitsprocessor].')
    ...
    # 假设你要抑制的 token 是 100（你需要根据实际情况替换）
    token_to_suppress = 12902  # 12902 是 'ки' 的 token id (for mistral models)
    logits_processor = CustomLogitsProcessor(token_to_suppress, strategy="zero")  # 或者 strategy="zero" avg
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        batch_outputs = model.generate(
            ...
            attention_mask=attention_mask,
            stopping_criteria=StoppingCriteriaList([stop_criteria]),
            logits_processor=[logits_processor],  # 传入自定义的 logits_processor
            **generation_kwargs
        )

...
def main():
    ...
    # **【修改点】** 針對 domain 數據集提取 QUERY 和 ANSWER
    domain_texts = load_texts(
        args.domain_dataset_path, 
        "domain", 
        args.num_samples, 
        lambda row: str(row['QUERY']) + tokenizer.eos_token + str(row['ANSWER'])
    )
    
    # Reasoning 數據集保持不變 (OpenThoughts 格式)
    reasoning_texts = load_texts(
        args.reasoning_dataset_path, 
        "reasoning", 
        args.num_samples, 
        lambda row: row['conversations'][0]['value'] + tokenizer.eos_token + row['conversations'][1]['value']
    )

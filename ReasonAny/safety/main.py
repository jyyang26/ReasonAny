...
def main():
    parser = argparse.ArgumentParser(description="Modified reasonany script with selectable strategy.")
    # (參數定義部分)
    ...
    parser.add_argument("--is_intersect", type=str, default=True)
    ...
    if not safety_texts or not reasoning_texts:
        print("[ERROR] Failed to load texts. Aborting.")
        return

    # ==================== 2. 安全模型處理 ====================
    print("\n--- 3. Processing Safety Model ---")
    ...
    # 對於 safety 模型，始終選擇最高分 (select_lowest=False)
    ...
    print("Releasing safety model and scores from memory...")
    del safety_model, safety_model_scores, base_on_safety_scores, N_safety, N_base_for_safety
    gc.collect()
    ...
    if args.is_intersect:
        ...
        T_reasoning = N_base_for_reasoning
    ...
    print(f"Disjoint Safety Params: {len(T_safety_disjoint)}")
